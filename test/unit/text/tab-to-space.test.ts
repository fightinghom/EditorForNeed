...
describe('editor.text event-hooks tab-to-space test', () => {
	...
	test('能绑定一个处理 tab 的函数，如果没有选区内容，则不执行后续的插入操作', () => {
		...
		jest.spyOn(editor.selection, 'getSelectionContainerElem').mockImplementation(() => undefined)
	...
	test('能绑定一个处理 tab 的函数，如果有选区内容，并且是正常的HTML元素，则插入空格', () => {
		...
		jest.spyOn(editor.selection, 'getSelectionContainerElem').mockImplementation(() => container)
	...
	describe('当选区内容父元素为code，pre，hljs或者选区元素为code的情况，则插入特殊的空格', () => {
		...
		const mockGetSelectionContainerElem = (tagString: string, isChild = true) => {
			...
			container.append($('<p>123</p>'))
			jest
				.spyOn(editor.selection, 'getSelectionContainerElem')
				.mockImplementation(() => (isChild ? container.children()! : container))
		}
		...
		test('选区元素是 CODE', () => {
			...
			expect(document.execCommand).toBeCalledWith('insertHTML', false, editor.config.languageTab)
		})

...
class LineHeight extends DropListMenu implements MenuActive {
		...
		if ($selectionElem && editor.$textElem.equal($selectionElem)) {
			...
			arrayDom_a.forEach(item => {
				...
				if ($(item).getNodeName() === 'P') {
					...
					st =
						st +
						`<${$(item).getNodeName().toLowerCase()} style="${styleStr}">${item.innerHTML}</${$(
							item
						)
							.getNodeName()
							.toLowerCase()}>`
		...
		if (!value) {
			if (style) {
				...
				if (styleStr === '') {
					...
					st = `<${$(dom).getNodeName().toLowerCase()} style="${styleStr}">${dom.innerHTML}</${$(
						dom
					)
						.getNodeName()
						.toLowerCase()}>`
	...
	public styleProcessing(styleList: Array<string>): string {
		...
		styleList.forEach(item => {
			item !== '' && item.indexOf('line-height') === -1 ? (styleStr = styleStr + item + ';') : ''
		})
		return styleStr
	}

...
class Indent extends DropListMenu implements MenuActive {
            ...
            list: [
                {
                    $elem: $(
                        `<p>
                            ...
                            ${editor.i18next.t('menus.dropListMenu.indent.增加段落缩进')}
                        <p>`
                    ),
                    value: 'increase',
                },
                // ----------------------- 增加首行缩进 yanghao ----------------------
                {
                    $elem: $(
                        `<p>
                            <i class="w-e-icon-indent-increase w-e-drop-list-item"></i>
                            ${editor.i18next.t('menus.dropListMenu.indent.增加首行缩进')}
                        <p>`
                    ),
                    value: 'flincrease',
                },
                // ----------------------- ------------------- ----------------------
                {
                    $elem: $(
                        `<p>
                            ...
                            ${editor.i18next.t('menus.dropListMenu.indent.减少段落缩进')}
                        <p>`
                    ),
                    value: 'decrease',
                },
                {
                    $elem: $(
                        `<p>
                            <i class="w-e-icon-indent-decrease w-e-drop-list-item"></i>
                            ${editor.i18next.t('menus.dropListMenu.indent.减少首行缩进')}
                        <p>`
                    ),
                    value: 'fldecrease',
                },
    ...
    public tryChangeActive(): void {
        ...
        if (
            $selectionStartElem.elems[0].style['paddingLeft'] != '' ||
            $selectionStartElem.elems[0].style['textIndent'] != ''
        ) {
            this.active()
        } else {
            this.unActive()

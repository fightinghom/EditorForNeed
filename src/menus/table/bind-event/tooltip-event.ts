		...
		const conf: TooltipConfType = [
			...
			},
			{
				...
				onClick: (editor: Editor, $node: DomElement) => {
					...
					const trLength: number = operatingEvent.DeleteRow($(htmlStr), index).elems[0].children[0]
						.children.length
					//生成新的table
					let newdom: string = ''
					// 选中table
					...
					if (trLength === 0) {
						...
						newdom = getnode.getTableHtml(operatingEvent.DeleteRow($(htmlStr), index).elems[0])
					}
					newdom = _isEmptyP($node, newdom)
					editor.cmd.do('insertHTML', newdom)
